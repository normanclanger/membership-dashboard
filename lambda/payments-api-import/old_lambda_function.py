...
def create_import_lines(event):
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Confirm the import exists and is still open.
            cur.execute(
                """
                SELECT
                    id,
...
def create_import_item(event):
    ...
    try:
        with conn.cursor() as cur:
            ...
            # Confirm the import is still open.
            cur.execute(
                """
                SELECT status
        ...
        return created({
            ...
            "allocated_amount": str(new_total),
...
def get_import(event):
    ...
    import_id = query_parameters.get("id")
...
def lambda_handler(event, context):
    ...
    if http_method == "POST" and not path_parameters:
        if not can_write_imports(event):
            return forbidden({
                "error": "You do not have permission to create payment imports"
            })
